...
export class CommonExpressionHelpers {
  ...
  getRadioScore = (questionId: string) => {
    console.log(`Calculating score for questionId: "${questionId}"`);
  
    const selectedValue = this.useFieldValue(questionId);
    console.log(`Selected value for "${questionId}":`, selectedValue);
  
    if (selectedValue === null || selectedValue === undefined) {
      console.log(`No value selected for "${questionId}", returning 0`);
      return 0;
    }
  
    const targetField = this.allFields.find((f) => f.id === questionId);
    console.log('Target field found:', targetField);
  
    if (!targetField || !targetField.questionOptions?.answers) {
      console.log(`No answers found for "${questionId}", returning 0`);
      return 0;
    }
  
    const answerObj = targetField.questionOptions.answers.find(
      (a: any) => a.value === selectedValue
    );
    console.log('Selected answer object:', answerObj);
  
    if (!answerObj) {
      console.log(`Selected value not found in answers for "${questionId}", returning 0`);
      return 0;
    }
  
    // **Map UI values to calculation value**
    let calcValue = 0; // default
    if (answerObj.value === 0) {
      calcValue = 0;
    }
  ...
  getRadioScoreNumeric = (questionId: string) => {
    console.log(`Calculating numeric score for questionId: "${questionId}"`);
    ...
    // Directly return the selected value
    console.log(`Numeric value for "${questionId}":`, answerObj.value);
    return answerObj.value;
  };

  scrollToRequiredField = (questionId: string) => {
    console.log(`scrollToRequiredField called for questionId: "${questionId}"`);
  
    // Try to find the field element by its name attribute
    const el = document.querySelector(`[name="${questionId}"]`) as HTMLElement | null;
  
    if (!el) {
      console.log(`Element not found for questionId: "${questionId}"`);
      return false;
    }
    ...
    if (isRequired && !isFilled) {
      console.log(`Field "${questionId}" is required but empty. Scrolling to it...`);
      el.scrollIntoView({ behavior: 'smooth', block: 'center' });
      el.focus?.();
      return true; // field was missing
    }
