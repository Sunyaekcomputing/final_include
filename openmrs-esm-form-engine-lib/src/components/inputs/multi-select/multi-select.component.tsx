        ...
        <div className={styles.boldedLabel}>
          <Layer>
            {isSearchable ? (
              ...
              {field.questionOptions.answers?.map((answer, index) => {
                const checkboxId = `${field.id}-${answer.concept}`;
                return (
                  <Checkbox
                    ...
                    name={field.id} // <--- all checkboxes share the field ID as name
                    value={answer.concept} // <-- add value prop
                    onChange={() => handleSelectCheckbox(answer)}
                    readOnly={isTrue(field.readonly)}
                    data-testid={`${field.id}-checkbox-${answer.label}`} // consistent for testing
                  />
                );
              })}
            </CheckboxGroup>
