...
export class EncounterFormProcessor extends FormProcessor {
  ...
  encounterDate instanceof Date
   ...
   if (!safeEncounterDate) missingFields.push('Encounter date');
    ...
    try {
      const programs = preparePatientPrograms(context.formFields, context.patient, context.customDependencies.patientPrograms);
      console.log('[DEBUG] Saving patient programs:', programs);
    ...
    try {
      ...
      const savedDiagnoses = savedEncounter.diagnoses.map((d) => d.display);
      if (savedOrders.length) console.log('[DEBUG] Orders saved:', savedOrders);
      if (savedDiagnoses.length) console.log('[DEBUG] Diagnoses saved:', savedDiagnoses);
